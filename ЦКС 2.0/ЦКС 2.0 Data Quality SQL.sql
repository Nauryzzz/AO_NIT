DROP TABLE IF EXISTS TEST.DATA_QUALITY;
CREATE TABLE TEST.DATA_QUALITY
(
    `ID` Nullable(String),
    `CHECK_DATE` Nullable(DateTime),
    `RULE_ID` Int8,
...
where 
	fl.CHANGE_TIME_NUM = 1 and 
	(cast(SUBSTRING(fl.BIRTH_DATE, 1, 4) as Int32) < 1800 or cast(SUBSTRING(fl.BIRTH_DATE, 1, 4) as Int32) > 2024);
...
where 
	birth.CHANGE_DATE_NUM = 1 and 
	birth.MOTHER_IIN is null;
...
where 
	birth.CHANGE_DATE_NUM = 1 and 
	birth.MOTHER_IIN = birth.CHILD_IIN;
...
where 
	birth.CHANGE_DATE_NUM = 1 and 
	birth.FATHER_IIN = birth.CHILD_IIN;
...
where 
	birth.CHANGE_DATE_NUM = 1 and 
	(birth.FATHER_IIN is null and birth.MOTHER_IIN is null);
...
where 
	birth.CHANGE_DATE_NUM = 1 and 
	birth.MOTHER_IIN = birth.FATHER_IIN;
...
where 
	marriage.CHANGE_DATE_NUM = 1 and -- Последняя измененная запись 
	marriage.DIVORCE_AKT_NUMBER is null and -- Брак не расторгнут
	(marriage.MAN_IIN is null and marriage.WOMAN_IIN is null);
...
where 
	marriage.CHANGE_DATE_NUM = 1 and -- Последняя измененная запись
	marriage.DIVORCE_AKT_NUMBER is null and -- Брак не расторгнут
	(marriage.MAN_IIN = marriage.WOMAN_IIN);
...
where 
	marriage.CHANGE_DATE_NUM = 1 and -- Последняя измененная запись 
	marriage.DIVORCE_AKT_NUMBER is null and -- Брак не расторгнут
	marriage.WOMAN_IIN is not null
