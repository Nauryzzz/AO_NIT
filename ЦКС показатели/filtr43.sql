...
select
	...
	'filtr43' as filtr, /* название показателя, необходимо для идентификации значений текущего показателя при объедений */
	if(count(p9.IIN) > 0, 1, 0) as filtr_value /* если в семье есть хоть один подходящий ИИН, то признак будет 1 иначе 0 */
from
	(select 
		distinct gp.IIN as IIN /* дети от 5 до 18 лет */
	...
	except /* ИИН детей от 5 до 18 лет которые отсутствуют в списке обучающихся */
