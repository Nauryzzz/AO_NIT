...
select
	...
	'filtr45' as filtr, /* название показателя, необходимо для идентификации значений текущего показателя при объедений */
...
from
	...
	from
		(select 
			distinct gp.IIN as IIN /* дети до 18 лет */
	...
	inner join /* объединение детей до 18 лет со списком сирот */
		...
		where e.IS_ORPHAN is not null and e.IS_ORPHAN <> 0 and /* признак сирота */
			st.IIN <> '4EE9CB68BAD1069BBE54103C9FBD957807CDE54A8B4BAC570A9326425D45E7B8' and 
			st.IIN is not null) as vt2 /* список детей сирот */
