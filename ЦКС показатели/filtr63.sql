/* 33. Информация по выпускникам колледжей и профессиональных школ */
select
	toString(fm.SK_FAMILY_ID) as SK_FAMILY_ID, -- ID семьи
	'filtr63' as filtr, -- Информация по выпускникам колледжей и профессиональных школ
	if(count(p33.IIN) > 0, 1, 0) as filtr_value -- если в семье есть хоть один подходящий ИИН, то признак будет 1 иначе 0
from
	(select distinct
		n_141.IIN,
		n_142.value
	from
		...
		where date_diff(year, toDate(gp.BIRTH_DATE), today()) >= 16 and 
			...
			gp.PERSON_STATUS_ID <> 3) as n_141
			  
		inner join -- объединение людей от 16 до 29 лет с людьми с высшим образованием
		
		(select distinct -- выпускники колледжей и профессиональных школ
			vt2.IIN as IIN,
		...
		from
			...
			from
				...
				where s.DATE_CLOSE1 is null and
					ts.ID = 3 /* Организации технического и профессионального образования */ and 
			...
			where vt2.REG_DATE is not null and vt2.OUT_DATE is not null) as n_142
		on n_141.IIN = n_142.IIN) as p33
	inner join SK_FAMILY.SK_FAMILY_MEMBER as fm on fm.IIN = p33.IIN -- определение ID семьи для ИИН
