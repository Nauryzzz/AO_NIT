...
select
	...
	'filtr58' as filtr, -- безработное лицо с ограниченной возможностью
	if(count(p44.IIN) > 0, 1, 0) as filtr_value
from
	(select
		gbl_pers.IIN as IIN
	from gbl_pers
	...
	where 
		...
		if(inv.IIN 			= '', 	null, inv.IIN)  		is not null and
		if(ofic_bezrab.IIN 	= '', 	null, ofic_bezrab.IIN)  is null) as p44
inner join SK_FAMILY.SK_FAMILY_MEMBER as fm on fm.IIN = p44.IIN
