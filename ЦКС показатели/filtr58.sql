/* 44. Безработное лицо с ограниченной возможностью */
with 
...
select
	...
	'filtr58' as filtr, -- необходимо для определения значений текущего показателя при UNION ALL
	if(count(p44.IIN) > 0, 1, 0) as filtr_value -- если в семье есть хоть один подходящий ИИН, то признак будет 1 иначе 0
from
	(select
		gbl_pers.IIN as IIN
	from gbl_pers
		...
		left join inv 									on inv.IIN 			= gbl_pers.IIN		
	...
	where 
		...
		if(inv.IIN 			= '', 	null, inv.IIN)  		is not null and -- есть инвалидность
		if(ofic_bezrab.IIN 	= '', 	null, ofic_bezrab.IIN)  is null -- не числится как официально безработный
		) as p44
inner join SK_FAMILY.SK_FAMILY_MEMBER as fm on fm.IIN = p44.IIN -- определение ID семьи для ИИН
