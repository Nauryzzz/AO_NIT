...
select
	...
	'filtr57' as filtr, -- безработный
	if(count(p46.IIN) > 0, 1, 0) as filtr_value
from
	(select
		gbl_pers.IIN as IIN
	from gbl_pers
	...
	where 
		...
		if(ofic_bezrab.IIN 	= '', 	null, ofic_bezrab.IIN)  is null) as p46
inner join SK_FAMILY.SK_FAMILY_MEMBER as fm on fm.IIN = p46.IIN
